...
onSubmitRegisterStudent() {
    if (this.studentSignupForm.valid) {
      ...
      const disclaimerAccepted = this.studentSignupForm.value.disclaimerAccepted || false;
      // Check if username or password is undefined or null
      if (typeof username === 'string' && typeof password === 'string' && disclaimerAccepted) {
...
onSubmitRegisterTeacher() {
  if (this.teacherSignupForm.valid) {
    ...
    const disclaimerAccepted = this.teacherSignupForm.value.disclaimerAccepted || false;
    // Check if username or password is undefined or null
    if (typeof username === 'string' && typeof password === 'string' && disclaimerAccepted) {
      this.authService.teacherSignup(username, email, firstName, lastName, password).subscribe({
        next: (user) => {
          if (user && user.token) {
          // Handle successful signup
          console.log('Signup successful');
